...
class Base_Op(object):
    ...
    def token_sent(self):
        cur=0
        data=pkl.load(open(os.path.join(self.opt.SPLIT_DATASET,self.opt.DATASET+'_new.pkl'),'rb'))             
    ...
    def create_fasttext(self):
        ...
        if self.opt.DATASET=='dt':
            ...
            np.save(os.path.join(self.opt.TOTAL_DATA,'fast_embedding.npy'),weights)
        return weights
    ...
    def create_para(self):
        ...
        if self.opt.DATASET=='dt':
            ...
            np.save(os.path.join(self.opt.TOTAL_DATA,'para_embedding.npy'),weights)
        return weights
    ...
    def create_senti(self):
        ...
        if self.opt.DATASET=='dt':
            ...
            np.save(os.path.join(self.opt.TOTAL_DATA,'senti_embedding.npy'),weights)
        return weights
    ...
    def init_dict(self):
        ...
        if self.opt.CREATE_EMB:
            ...
            if self.opt.DATASET=='dt':
                ...
                self.fast_weights=np.load(os.path.join(self.opt.TOTAL_DATA,'fast_embedding.npy'))
                self.para_weights=np.load(os.path.join(self.opt.TOTAL_DATA,'para_embedding.npy'))
                self.senti_weights=np.load(os.path.join(self.opt.TOTAL_DATA,'senti_embedding.npy'))
        self.ntoken()
...
class Wraped_Data(Base_Op):
    def __init__(self,opt,dictionary,split_data,test_num,mode='training'):
        ...
        if self.opt.DATASET=='dt' or self.opt.DATASET=='total':
            self.classes=2
        elif self.opt.DATASET=='dt_full':
            self.classes=3
        else:
    ...
    def load_tr_val_entries(self):
        ...
        for info in all_data:
            ...
            if self.opt.DATASET=='total':
                label=info['answer']
                """
                the length of topic vector is not the same in TOTAL dataset
                pad zeros to make the vector the same length as the maximum length vector
                """
                prob=self.padding_prob(info['prob'])
            else:
                label=info['label']
                prob=info['topic']
            entry={
    ...
    def padding_prob(self,prob):
        length=prob.shape[0]
        if length<self.opt.NUM_TOPICS:
            padding =np.zeros(self.opt.NUM_TOPICS-length,)
            prob=np.concatenate((prob,padding),axis=0)
        return prob
