...
class SparseLDA:
    def __init__(self, num_topics, entropy_reg=0.5, burning_period=100, max_iterations=1, sampling_gap=20):
    ...
    def sample(self, d, i, w, vocab_size):
        ...
        weights = np.array([self.n_dz[d, z] * (self.n_zw[z, w] + self.beta).astype(np.float64) / (self.sum_nzw[z] + self.beta * vocab_size)
                            for z in range(self.num_topics)])

        #print('weights.shape = ', weights)
        curr_entropy = compute_entropy(self.n_dz[d])
        #print('curr_entropy.shape = ', curr_entropy)
        entropies = np.array([update_entropy(self.n_dz[d, z], 1, self.sum_ndz[d], curr_entropy)
                              for z in range(self.num_topics)])
        #print('entropies.shape = ', entropies)
        regs = np.exp(-entropies * entropies / (2 * self.entropy_reg))
        #print('regs.shape = ', regs)
